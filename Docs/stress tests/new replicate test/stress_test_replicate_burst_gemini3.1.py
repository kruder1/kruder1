"""
Replicate Nano Banana 2 - Stress Test BURST (50 Concurrentes)
Lanza 50 peticiones simultáneas con el payload EXACTO de google/nano-banana-2.
...
# Endpoint oficial para google/nano-banana-2
...
LOG_PATH = SCRIPT_DIR / "log_burst_replicate_nano.txt"
...
def send_and_poll(idx_1based: int, prompt: str, log_file) -> dict:
    # Payload EXACTO para Nano Banana 2
    body = {
        "input": {
            "prompt": prompt,
...
def main():
    ...
    log_write(log_file, f"[{ts()}] ========== REPLICATE NANO BANANA 2 BURST STRESS TEST ==========")
