"""
Segmind Seedream 4.5 - Stress Test BURST
Lanza 30 requests en paralelo. Todo se registra en log_burst_segmind.txt.
...
LOG_PATH = SCRIPT_DIR / "log_burst_segmind.txt"
    ...
    try:
        ...
        with urlopen(req, timeout=REQUEST_TIMEOUT) as r:
            ...
            if status == 200:
                ...
                log_write(log_file, f"[{ts()}] REQ #{idx_1based} response_body_full={body_text}")
            ...
            return {
                ...
                "body_size": body_len,
                "error": None,
                "body_preview": body_text if status != 200 else None,
            }
...
def main():
    # Abrir log al principio para no perder nada
    try:
    ...
    log_write(log_file, f"[{ts()}] ========== SEGMIND BURST STRESS TEST ==========")
    log_write(log_file, f"[{ts()}] endpoint={ENDPOINT}")
    log_write(log_file, f"[{ts()}] image_url={IMAGE_URL}")
    log_write(log_file, f"[{ts()}] num_requests={NUM_REQUESTS} mode=burst")
    ...
    log_write(log_file, f"[{ts()}] Cargados {len(prompts_to_use)} prompts (primeros {NUM_REQUESTS} del JSON). Iniciando envío paralelo.")
    log_write(log_file, "")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_REQUESTS) as ex:
        ...
        for f in concurrent.futures.as_completed(futures):
            try:
                ...
                log_write(log_file, f"[{ts()}] FUTURE EXCEPTION: {ex}\n{traceback.format_exc()}")
                # Marcar como fallo sin índice si no tenemos resultado
                results.append({
                    "idx": None,
                    "status": None,
                    "elapsed_ms": 0,
